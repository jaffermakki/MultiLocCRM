...
@bp.route("/receipt/<int:invoice_id>/thermal")
@login_required
def thermal_receipt(invoice_id):
    """80mm thermal roll version of the same invoice - same totals and line items
    as the full invoice page, formatted to actually print correctly on a real
    thermal printer (see @page rule in app.css) rather than just looking narrow
    on screen."""
    from app.models.invoice import Invoice

    invoice = Invoice.query.get_or_404(invoice_id)
    settings = ShopSettings.get()
    return render_template("print/thermal_receipt.html", invoice=invoice, settings=settings)


@bp.route("/receipt/<int:invoice_id>/email", methods=["POST"])
@login_required
def email_receipt(invoice_id):
    from app.models.invoice import Invoice
    from app.services.notifications import send_receipt_email
    from app.services.notifications.base import NotificationError

    invoice = Invoice.query.get_or_404(invoice_id)

    if not invoice.customer or not invoice.customer.email:
        flash("This invoice has no customer email on file.", "amber")
        return redirect(url_for("pos.receipt", invoice_id=invoice.id))

    try:
        sent = send_receipt_email(invoice)
    except NotificationError as exc:
        flash(f"Couldn't send the email: {exc}", "red")
        return redirect(url_for("pos.receipt", invoice_id=invoice.id))

    if sent:
        flash(f"Receipt emailed to {invoice.customer.email}.", "green")
    else:
        flash("Email receipts aren't enabled in Settings.", "amber")
    return redirect(url_for("pos.receipt", invoice_id=invoice.id))
