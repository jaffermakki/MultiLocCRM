...
def save_email():
    ...
    settings.email_provider = request.form.get("email_provider", "smtp")
    settings.email_from_address = request.form.get("email_from_address", "").strip()
    settings.email_from_name = request.form.get("email_from_name", "").strip()
    settings.email_receipt_subject = request.form.get("email_receipt_subject", "").strip() or "Your receipt from {shop_name}"
    db.session.commit()
...
@role_required("owner", "manager")
def test_email():
    from app.services.notifications import get_email_provider
    from app.services.notifications.base import NotificationError

    settings = ShopSettings.get()
    test_address = request.form.get("test_email_address", "").strip()
    if not test_address:
        flash("Enter an email address to send the test to.", "amber")
        return redirect(url_for("settings.index", tab="email"))
    ...
    try:
        ...
        provider.send(
            ...
            to_name="",
            subject=f"Test email from {settings.shop_name}",
            html_body=f"<p>This is a test email from {settings.shop_name}'s Tech-Pro+ CRM. If you're reading this, your email provider is configured correctly.</p>",
        )
    except NotificationError as exc:
        flash(f"Test email failed: {exc}", "red")
        return redirect(url_for("settings.index", tab="email"))

    flash(f"Test email sent to {test_address}.", "green")
    return redirect(url_for("settings.index", tab="email"))
...
@role_required("owner", "manager")
def test_sms():
    from app.services.notifications import get_sms_provider
    from app.services.notifications.base import NotificationError

    settings = ShopSettings.get()
    test_number = request.form.get("test_phone_number", "").strip()
    if not test_number:
        flash("Enter a phone number to send the test to.", "amber")
        return redirect(url_for("settings.index", tab="sms"))

    try:
        provider = get_sms_provider(settings)
        provider.send(to_number=test_number, body=f"Test SMS from {settings.shop_name}'s Tech-Pro+ CRM.")
    except NotificationError as exc:
        flash(f"Test SMS failed: {exc}", "red")
        return redirect(url_for("settings.index", tab="sms"))

    flash(f"Test SMS sent to {test_number}.", "green")
    return redirect(url_for("settings.index", tab="sms"))
