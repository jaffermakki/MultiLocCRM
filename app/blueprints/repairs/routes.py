...
from app.services.notifications import send_repair_ready_sms
...
def advance(repair_id):
    ...
    if next_status == "READY":
        try:
            sent = send_repair_ready_sms(repair)
            if sent:
                flash(f"Ready-for-pickup SMS sent to {repair.customer.name}.", "blue")
        except NotificationError as exc:
            # SMS failing should never block the status change itself - the repair
            # IS ready, the notification just didn't go out. Surface it as a
            # separate warning so staff know to call the customer instead.
            flash(f"Status updated, but the SMS failed to send: {exc}", "amber")
