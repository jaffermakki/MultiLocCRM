...
class ShopSettings(db.Model):
    ...
    # Email tab - provider credentials (API keys, SMTP passwords) live in .env, never
    # in the database, since this table is plain-readable to anyone with DB access.
    # Only the provider CHOICE and non-secret config (from-address, sender name) live
    # here, so switching providers is a Settings change, not a code change.
    email_receipts_enabled = db.Column(db.Boolean, default=False)
    email_provider = db.Column(db.String(20), default="smtp")  # smtp, sendgrid, mailgun, ses, postmark
    email_from_address = db.Column(db.String(160), default="")
    email_from_name = db.Column(db.String(160), default="")
    email_receipt_subject = db.Column(db.String(160), default="Your receipt from {shop_name}")

    # SMS tab - same pattern: provider choice + non-secret config here, API
    # credentials in .env.
    sms_enabled = db.Column(db.Boolean, default=False)
    sms_provider = db.Column(db.String(20), default="twilio")  # twilio, vonage, messagebird, telnyx, aws_sns
    sms_from_number = db.Column(db.String(40), default="")
